...
const EventDetail: React.FC = () => {
  ...
  if (loading) return <div className="flex justify-center items-center h-screen">Loading event...</div>;
  if (!event) return <div className="flex justify-center items-center h-screen text-red-500">Event not found</div>;
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <article className="bg-white rounded-2xl shadow-sm p-6 md:p-10 mb-8">
          <div className="mb-8 pb-6 border-b border-gray-100">
            <div className="text-sm font-bold text-[#4682dc] uppercase tracking-wider mb-3">{event.category}</div>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 text-sm text-gray-500">
              ...
              <span className="font-medium text-gray-700">By <Link to={`/author/${event.authorId}`} className="hover:text-[#4682dc] transition-colors">{event.author}</Link></span>

              <div className="flex items-center gap-4 mt-2 md:mt-0 md:ml-auto">
                <span className="flex items-center gap-1 text-red-500"><Heart className="w-4 h-4 fill-current" /> {event.likes}</span>
                <span className="flex items-center gap-1 text-yellow-500"><Star className="w-4 h-4 fill-current" /> {event.favorites}</span>
                <span className="flex items-center gap-1 text-blue-500"><Eye className="w-4 h-4" /> {event.views}</span>
                <span className="flex items-center gap-1 text-green-500"><MessageCircle className="w-4 h-4" /> {event.comments.length}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 rounded-2xl overflow-hidden h-64 md:h-[400px] relative bg-gray-100">
            {event.image ? (
              <img src={`/images/${event.image}`} alt={event.title} className="w-full h-full object-cover" />
            ) : (
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-xl flex flex-wrap items-center gap-3">
            ...
            <button className="ml-auto text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-50 transition-colors" title="Delete Event">
          ...
          <section className="mt-12 pt-8 border-t border-gray-100">
            <h3 className="text-2xl font-bold text-[#4682dc] mb-6 flex items-center gap-2">
              <MessageCircle className="w-6 h-6" /> Comments ({event.comments.length})
            </h3>

            {/* Add comment form */}
            <form onSubmit={handleAddComment} className="bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-100">
              <div className="mb-4">
                <label htmlFor="comment_author" className="block text-sm font-medium text-gray-700 mb-2">Your name:</label>
                <input
                  type="text"
                  id="comment_author"
                  value={commentAuthor}
                ...
                event.comments.map(comment => (
                  <div key={comment.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-2">
                      <strong className="text-[#4682dc] font-bold text-lg">{comment.author}</strong>
                      <span className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-full">{comment.date}</span>
                    </div>
